pub fn main() {
    ...
    println!(
        "Filepath (C:\\Users\\User\\Desktop\\file.txt) -> (C:\\\\Users\\\\User\\\\Desktop\\\\file.txt)...\n or a raw string (r\"C:\\Users\\User\\Desktop\\file.txt\") -> {}",
        raw_string
    );
    ...
    println!(
        "&str formatted into a String: String::from(&str): {}",
        String::from(world)
    );
    println!("String into a &str: &String[..]: {}", &string[..]);

    println!(
        "String split by whitespace: {:?} (str > iterator > collection)",
        string.split(" ").collect::<Vec<&str>>()
    );

    println!(
        "String replace 'world' with 'there': {}",
    ...
    println!(
        "Custom Pad (:number>variable$):{number:1>width$}",
        number = "0",
        width = 5
    );
