...
pub fn main() {
    ...
    println!("  - It is also the most difficult string type to work with because of its lack of ownership and mutability.");
    ...
    println!(" let text_food: String = String::from(food); // text_food: {text_food} -> referencia a food");
    ...
    println!("Memory prepared for name (before pushing): {:?}", name.capacity());
    ...
    println!("  - str: A hardcoded, read-only piece of text encoded in the binary, never will see in the code cause is already the codifier text.");
