pub fn main() {
    println!("\nCopy");
    println!(
        "  - Copy is a trait that allows a type to be copied by simply assigning it to another variable, allowing his future uses without cloning or referencing."
    );
    println!(
        "  - Copy is only implemented for a few types, such as integers, booleans, REFERENCES, and characters."
    );
    ...
    println!(
        "  - vec_or_mut_arr_copy: {vec_or_mut_arr_copy:#?} change of ownership, needs to be copied with clone()"
    );
