...
fn explication(name_fn: &str) {
    println!("\nFn {name_fn}:");
    println!(
        "  ...PASO como ARGUMENTO una referencia mutable del valor mutable para mo perder el ownership"
    );
    println!(
        "  ...RECIBO como ARGUMENTO una referencia mutable del valor mutable (lo mismo que paso)"
    );
    println!(
        "  ...no necesito devolverlo, el scope de la memoria es global como RETORNO una referencia mutable del valor mutable (lo mismo que paso y recibo)\n"
    );
}
