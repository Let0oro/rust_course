pub fn main() {
    ...
    println!(
        " - Is an unreliable references, means that is a reference to a value that is no longer present."
    );
    println!(
    ...
    println!(
        "\"(text fn) fn create_city -> &String (city) > let ref = create_city()\" - The existence of the city value has been cleared."
    );
