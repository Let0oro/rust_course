...
pub fn main () {
  ...
  println!(" - A design patter is a recommended way to write or structure code to solve specific problems");

  let mut computer = Computer::new(String::from("M3 Max"), 64, 2);

  println!("My Computer Struct -> {:#?}", computer);
  println!("Shoud be better to add Computer upgrades (or set) to all namespaces or values defining on his struct");
  println!(" * If we return 'self' at every upgrade fn, we can upgrade concatenally the values of our Computer instances");
