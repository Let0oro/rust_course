...
impl TaylorSwiftSong {
    ...
    pub fn display_song_info(self: &Self) {
        ...
        println!("\nTitle: {}", self.title);
        println!(
            "Release Year: {}, so it was released {} year{} ago",
            self.release_year, &years_passed, sing_plur
        );
...
pub fn main() {
  ...
  The solution is: 'name: mocha.name.clone(), ..mocha'");
    ...
    println!(
        "Deriving our structs for implement the Display o Debug trait on there adding '#[derive(Debug)]' at he first file line"
    );

    println!("{:#?}", latte);

    println!(
        " - In Rust, the states or values are defined in the 'struct' (structure) curly brackets, and the methods are defined in the 'impl <struct_name>' (implementation) curly brackets"
    );
